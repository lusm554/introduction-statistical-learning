...
### 1. **t-статистика**
t-статистика используется в линейной регрессии для проверки значимости отдельных коэффициентов модели. Она показывает, насколько сильно коэффициент отличается от нуля в стандартных ошибках.

#### Формула t-статистики:
Для коэффициента \( \beta_j \) (где \( j \) — индекс предиктора):
...
- \( \text{SE}(\hat{\beta}_j) \) — стандартная ошибка коэффициента.

#### Интерпретация:
- Чем больше абсолютное значение t-статистики, тем сильнее доказательства против нулевой гипотезы \( H_0: \beta_j = 0 \).
- Если \( |t| > 2 \), это обычно указывает на значимость коэффициента на уровне значимости 5%.

---

### 2. **p-value**
p-value — это вероятность получить значение t-статистики, такое же или более экстремальное, чем наблюдаемое, при условии, что нулевая гипотеза \( H_0 \) верна.

#### Как вычисляется p-value:
p-value вычисляется на основе t-распределения с \( n - p - 1 \) степенями свободы, где:
- \( n \) — количество наблюдений,
- \( p \) — количество предикторов.

#### Интерпретация:
- **p-value < 0.05**: Коэффициент считается статистически значимым на уровне 5%. Мы отвергаем нулевую гипотезу \( H_0: \beta_j = 0 \) и заключаем, что предиктор \( X_j \) значимо влияет на \( y \).
- **p-value ≥ 0.05**: Коэффициент не является статистически значимым. Мы не можем отвергнуть нулевую гипотезу \( H_0 \).

---

### Связь между t-статистикой и p-value
- Чем больше абсолютное значение t-статистики, тем меньше p-value.
- Если t-статистика близка к нулю, p-value будет большим, что указывает на незначимость коэффициента.

---

### Важные замечания
1. **Уровень значимости**:
   - Уровень значимости (обычно 0.05) — это порог, ниже которого p-value считается достаточно малым для отвержения нулевой гипотезы.

2. **Множественное тестирование**:
   - Если в модели много предикторов, вероятность ложного обнаружения значимости (ошибка I рода) увеличивается. В таких случаях используют поправки, например, метод Бонферрони.

3. **Интерпретация в контексте**:
   - Даже если коэффициент статистически значим, важно оценить его практическую значимость (например, насколько велик его эффект).
