...
#### Интерпретация:
- Чем больше абсолютное значение t-статистики, тем сильнее доказательства против нулевой гипотезы.
- Если |t|>2, это обычно указывает на значимость коэффициента на уровне значимости 5%.

---

### 2. **p-value**
p-value — это вероятность получить значение t-статистики, такое же или более экстремальное, чем наблюдаемое, при условии, что нулевая гипотеза верна.

#### Интерпретация:
- **p-value < 0.05**: Коэффициент считается статистически значимым на уровне 5%. Мы отвергаем нулевую гипотезу и заключаем, что предиктор X_j значимо влияет на y.
- **p-value ≥ 0.05**: Коэффициент не является статистически значимым. Мы не можем отвергнуть нулевую гипотезу.
